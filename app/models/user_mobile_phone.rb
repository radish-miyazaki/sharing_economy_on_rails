class UserMobilePhone < ApplicationRecord
  ...
  validates :number, presence: true, uniqueness: true, format: { with: MOBILE_PHONE_REGEXP }, allow_blank: true
  validates :auth_code, presence: true

  before_validation :set_auth_code, if: :new_record?

  def send_auth_code_to_sms
    TwilioClient.new.send_auth_code_to_sms(to: number, auth_code: auth_code)
  end
